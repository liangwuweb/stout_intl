...
class Paginator {
    ...
    $rs = $this->_conn->query( $this->_query );//conn->query is actualy mysqli_query($db, sql)
    ...
    public function getData( $limit = 10, $page = 1 ) { //set default argument values
        ...
        //After pass the mysqli row to $results, free mysqli row
        $rs ->free();
        
        //print_r($results);die;

        //return data as object, new stdClass() creates new empty object ($result)
        $result         = new stdClass();
        $result->page   = $this->_page;
        $result->limit  = $this->_limit;
        $result->total  = $this->_total;
        //This stores the the rows from db
    ...
    public function createLinks( $links, $list_class ) 
    {
        ...
        //calculate start of range for link printing, strat link
        $start = ( ( $this->_page - $links ) > 0 ) ? $this->_page - $links : 1;
        
        //calculate end of range for link printing, end link
        ...
        '<li class="' . $class . ' page-item"><a class="page-link" href="">Previous</a></li>' : //remove link from previous button
        '<li class="' . $class . ' page-item"><a class="page-link" href="?limit=' . $this->_limit . '&page=' . ( $this->_page - 1 ) . '">Previous</a></li>';
        ...
        if ( $start > 1 ) { //print ... before (previous <<< link)
            ...
            $html .= '<li class="disabled page-item"><span>...</span></li>'; //print 3 dots if not on first page
        }
        ...
        if ( $end < $last ) { //print ... before next page (>>> link)
            $html .= '<li class="disabled page-item"><span>...</span></li>'; //print 3 dots if not on last page
        ...
        '<li class="' . $class . ' page-item"><a class="page-link" href="">Next</a></li>' : //remove link from next button
        '<li class="' . $class . ' page-item"><a class="page-link" href="?limit=' . $this->_limit . '&page=' . ( $this->_page + 1 ) . '">Next</a></li>';
